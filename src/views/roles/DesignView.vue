  ...
  >
    ...
    <template v-slot:[`item.design`]="{ item }">
      <v-btn
        class="takeOrder"
        :style="{
          backgroundColor: item.design == 0 ? '#2176ff' : '#32d47b'
        }"
        @click="takeOrder(item.design, item.id)"
        >{{ item.design == 0 ? 'Взять заказ' : 'Заказ взят' }}</v-btn
      >
    </template>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const headers = [
      ...
      { title: 'Статус', key: 'design', sortable: false }
    ]
