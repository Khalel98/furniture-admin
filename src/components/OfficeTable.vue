  ...
  >
    ...
    <template v-slot:[`item.${position}`]="{ item }">
      <v-btn
        class="takeOrder"
        :style="{
          backgroundColor: item[position] == 0 ? '#2176ff' : '#32d47b'
        }"
        @click="takeOrder(item[position], item.id)"
        >{{ item[position] == 0 ? 'Взять заказ' : 'Заказ взят' }}</v-btn
      >
    </template>
...
export default {
  ...
  setup(props) {
    ...
    const headers = [
      ...
      { title: 'Статус', key: `${position}`, sortable: false }
    ]
